...
public class MyoGattCallback extends BluetoothGattCallback {
    ...
    private MyoCallbackDelegate delegate;
    private String TAG = "MyoGatt";

    private TextView dataView;
    private String callback_msg;
    private Handler mHandler;
    private static final int  NUMBER_OF_EMG_DATA = 16;
    private static  final int NUMBER_OF_IMU_DATA = 20;
    private int[] emgDatas = new int[NUMBER_OF_EMG_DATA];
    //private int[] imuDatas = new int[NUMBER_OF_IMU_DATA];
    private byte[] imuByteDatas = new byte[NUMBER_OF_IMU_DATA];
    private byte[] emgByteDatas = new byte[NUMBER_OF_EMG_DATA];
    ...
    public void onServicesDiscovered(BluetoothGatt gatt, int status) {
        ...
        if (status == BluetoothGatt.GATT_SUCCESS) {
            ...
            if (service_imu == null) {
                ...
                Log.d(TAG, "Found Myo Imu Service");
                BluetoothGattCharacteristic characteristic = service_imu.getCharacteristic(UUID.fromString(IMU_0_ID));
                mCharacteristic_imu = characteristic;
                if (mCharacteristic_imu == null) {
                    Log.d(TAG, "Did not found the imu characteristic service");
                } else {
                    // Setting the notification
                    registerGattCharacteristicNotificationIfNeeded(gatt, mCharacteristic_imu, CLIENT_CHARACTERISTIC_CONFIG);
                }
            }
...
*/
                ...
                if (mCharacteristic_command == null) {
                    ...
                    /*mHandler.postDelayed(new Runnable() {
                        @Override
                        public void run() {
                            //delegate.didSetCharacteristicCommand(mCharacteristic_command);
                        }
                    }, 4000);*/
                    //delegate.didSetCharacteristicCommand(mCharacteristic_command);
                }
    ...
    private void registerGattCharacteristicNotificationIfNeeded(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, String uuid){
        boolean registered = gatt.setCharacteristicNotification(characteristic, true);
        if (registered){
            registerGattCharacteristicNotification(characteristic, uuid);
        } else {
            Log.d(TAG, "Could not set characteristic notification for characteristic: " + uuid);
        }
    }

    private void registerGattCharacteristicNotification(BluetoothGattCharacteristic characteristic, String uuid) {
        ...
        if (descriptor != null){
            ...
            Log.d(TAG, "Did set Descriptor for characteristic: " + uuid);
        } else {
            Log.d(TAG, "Did not found a characteristic for: " + uuid);
        }
    }
        ...
        if (status == BluetoothGatt.GATT_SUCCESS) {
            if (UUID.fromString(FIRMWARE_ID).equals(characteristic.getUuid())) {
                ...
                if (data != null && data.length > 0) {
                    ...
                    if (delegate != null){
                        delegate.didSetCharacteristicCommand(characteristic);
                    }
    ...
    public void onCharacteristicChanged(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
        Log.v("callback", "values: " + characteristic.getValue());
        if (EMG_0_ID.equals(characteristic.getUuid().toString())) {
            ...
            if (imuByteDatas != null && imuByteDatas.length != 0) {
                GestureDetectModelManager.getCurrentModel().event(systemTime_ms,emg_data, imuByteDatas);
            }

            ByteReader emg_br = new ByteReader();
            emg_br.setByteData(emg_data);
            Log.i("emgdata", emg_br.getByteDataString());
            ...
            emg_br.setByteData(emg_data);
            for(int emgInputIndex = 0;emgInputIndex<NUMBER_OF_EMG_DATA;emgInputIndex++) {
                byte currentByte = emg_br.getByte();
                emgDatas[emgInputIndex] = currentByte;
                emgByteDatas[emgInputIndex] =  currentByte;
            }
        ...
        } else if (UUID.fromString(IMU_0_ID).equals(characteristic.getUuid())) {
            byte[] imuData = characteristic.getValue();
            ByteReader br = new ByteReader();
            br.setByteData(imuData);
            for (int index = 0; index < NUMBER_OF_IMU_DATA; index++){
                //imuDatas[index] = br.getShort();
                //imuDatas[index] = br.getByte();
                imuByteDatas[index] = br.getByte();
            }
    ...
    public BluetoothGattCharacteristic getCharacteristicCommand(){
        return mCharacteristic_command;
    }

    public byte[] getImuByteDatas() {
        return imuByteDatas;
    }

    public byte[] getEmgByteDatas() {
        return emgByteDatas;
    }

    public MyoCallbackDelegate getDelegate() {
        return delegate;
    }

    public void setDelegate(MyoCallbackDelegate delegate) {
